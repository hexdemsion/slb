...
var req = {"id_tool": null, "hole": {"uphole": {"conn": null, "rop2usc": null, "crossover": null}, "downhole": {"conn": null, "rop2usc": null, "crossover": null} } }

req.id_tool = 5
req.hole.uphole.conn = "pin"
req.hole.uphole.rop2usc = 233
req.hole.uphole.crossover = 14.5

req.hole.downhole.conn = "box"
req.hole.downhole.rop2usc = 69.225
req.hole.downhole.crossover = 0
...
function get_extender_type(id_tool, hole, fin_len) {
	if (hole == "uphole") {
		...
		for (var i = 0; i < hole_list.length; i++) {
			...
			if ((fin_len-tool.min)*(fin_len-tool.max)<0 || (fin_len-tool.max)*(fin_len-tool.tolerance)<0) {
				final_ext.push(tool)
			}
		}
